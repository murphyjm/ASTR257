...
def fix_bad_col(data, bad_col = 256):
    '''
    Replace the bad column value with the average of the columns on either side.
    '''
    replace_with = np.nanmean(np.stack((data[:, bad_col - 1], data[:, bad_col + 1]), axis=1), axis=1)
    data[:, bad_col] = replace_with
